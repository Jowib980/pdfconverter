...
function SplitGeneratedPDF({ files = [] }) {
  ...
  const Convert = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}split`, {
        method: "POST",
        body: formData,
      });
            ...
            <div className="w-[350px] bg-white border-l border-gray-200 flex flex-col justify-between">
              ...
              <div className="relative flex flex-1 items-center justify-center overflow-hidden bg-gray-500">
                ...
                <div className="absolute inset-0 p-6 opacity-20 pointer-events-none">
                  <h1 className="text-xl font-semibold text-center tool-heading">Split PDF</h1>
...
export default SplitGeneratedPDF;
